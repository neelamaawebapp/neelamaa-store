...
import { X, MessageSquare, Wallet, ShoppingBag, TrendingUp, HelpCircle, Gift, Sparkles, ChevronLeft, Package, Truck, RefreshCcw, Headset, ArrowLeft } from "lucide-react";
...
export default function MascotAssistant() {
  ...
  const handleClaimBirthday = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || "Failed to claim birthday gift.");
      }
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div className="absolute left-0 bottom-20 w-[310px] bg-white/95 backdrop-blur-md border border-pink-100 rounded-3xl shadow-2xl shadow-pink-500/10 p-4 animate-fade-in-up z-50 text-left overflow-hidden flex flex-col h-auto max-h-[460px]">
          ...
          <div className="flex items-center gap-2 border-b border-gray-100 pb-2 mb-2 shrink-0">
            {viewMode === "chat" ? (
              <button
                ...
                className="p-1 rounded-full hover:bg-pink-50 text-pink-600 transition-colors flex items-center justify-center mr-0.5"
                title="Back to Options Menu"
              >
                <ArrowLeft size={16} />
              </button>
            ) : (
              <div className="w-9 h-9 rounded-full border border-pink-400/30 bg-pink-50 overflow-hidden shrink-0 flex items-center justify-center bg-gradient-to-br from-pink-500/10 to-orange-500/10">
                <img
                  src="/mascot/aarohi_waving.png"
                  alt="Aarohi mascot"
            ...
            <div className="flex-1">
              <h3 className="font-extrabold text-xs text-gray-900 tracking-wide">
                {viewMode === "menu" ? "Aarohi Assistant" : "Aarohi Chatbot"}
              </h3>
            ...
            <button 
              ...
              className="text-pink-600 hover:text-pink-700 hover:bg-pink-50 p-1.5 rounded-full transition-colors cursor-pointer flex items-center justify-center"
          ...
          {/* VIEW 1: Quick Options Menu */}
          {viewMode === "menu" ? (
            <div className="flex-1 overflow-y-auto pr-0.5 space-y-2 text-xs scrollbar-thin scrollbar-thumb-pink-100 max-h-[360px]">
              
              <div className="p-2 bg-gradient-to-r from-pink-50 via-rose-50 to-orange-50 rounded-2xl border border-pink-100/70 mb-2">
                <p className="text-[11px] font-extrabold text-gray-800 leading-snug">
                  Hi! 👋 How can I help you today?
                </p>
                <p className="text-[10px] text-gray-500 font-medium mt-0.5">
                  Select an option below or tap <span className="font-bold text-pink-600">Chat</span> to ask anything.
                </p>
              </div>

              {/* Birthday Banner in Menu if eligible */}
              {user && !birthdayVal && !birthdayGiftClaimed && (
                <div className="p-2.5 rounded-2xl bg-gradient-to-r from-pink-500/10 via-rose-500/10 to-orange-500/10 border border-pink-200/80 mb-2 select-none flex flex-col gap-1.5 relative">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-1">
                      <Gift size={13} className="text-pink-600 animate-bounce" />
                      <span className="text-[10px] font-black text-pink-700 uppercase tracking-wider">₹200 Birthday Reward!</span>
                    </div>
                  </div>
                  <p className="text-[9px] text-slate-650 font-semibold leading-tight">
                    Enter DOB to claim ₹200 instant wallet credits!
                  </p>
                  
                  {showBirthdaySuccess ? (
                    <div className="flex items-center gap-1 text-[9px] text-green-700 font-bold bg-green-50 p-1.5 rounded-lg border border-green-200">
                    ...
                    <div className="flex gap-1.5 items-center mt-0.5">
                      <input
                        ...
                        className="flex-1 border border-pink-200 rounded-lg px-2 py-1 text-[10px] text-gray-900 bg-white outline-none focus:ring-1 focus:ring-pink-500 font-semibold"
                      ...
                      <button
                        ...
                        className="bg-pink-500 hover:bg-pink-600 text-white font-extrabold px-2.5 py-1 rounded-lg text-[9px] uppercase tracking-wide cursor-pointer flex-shrink-0 disabled:opacity-50"
                      >
              ...
              <div className="grid grid-cols-1 gap-1.5">
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2.5 p-2.5 rounded-2xl bg-slate-50 hover:bg-pink-50/70 border border-slate-200/70 hover:border-pink-200 transition-all text-left group cursor-pointer"
                >
                  <div className="w-8 h-8 rounded-xl bg-pink-100/60 text-pink-600 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                    <Package size={16} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-xs text-gray-900 group-hover:text-pink-600 transition-colors">📦 Track Order</h4>
                    <p className="text-[9px] text-gray-500 truncate font-medium">View active order status & tracking info</p>
                  </div>
                  <ChevronLeft size={14} className="text-gray-400 rotate-180 group-hover:text-pink-600 transition-colors" />
                </button>
                ...
                <button
                  onClick={() => {
                    setViewMode("chat");
                    handleSendMessage("referral rewards");
                  }}
                  className="w-full flex items-center gap-2.5 p-2.5 rounded-2xl bg-slate-50 hover:bg-pink-50/70 border border-slate-200/70 hover:border-pink-200 transition-all text-left group cursor-pointer"
                >
                  <div className="w-8 h-8 rounded-xl bg-amber-100/60 text-amber-600 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                    <Gift size={16} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-xs text-gray-900 group-hover:text-pink-600 transition-colors">🎁 Rewards & Wallet</h4>
                    <p className="text-[9px] text-gray-500 truncate font-medium">Check balance & referral bonuses</p>
                  </div>
                  <ChevronLeft size={14} className="text-gray-400 rotate-180 group-hover:text-pink-600 transition-colors" />
                </button>
                ...
                <button
                  onClick={() => {
                    setViewMode("chat");
                    handleSendMessage("shipping charge");
                  }}
                  className="w-full flex items-center gap-2.5 p-2.5 rounded-2xl bg-slate-50 hover:bg-pink-50/70 border border-slate-200/70 hover:border-pink-200 transition-all text-left group cursor-pointer"
                >
                  <div className="w-8 h-8 rounded-xl bg-blue-100/60 text-blue-600 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                    <Truck size={16} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-xs text-gray-900 group-hover:text-pink-600 transition-colors">🚚 Shipping Rules</h4>
                    <p className="text-[9px] text-gray-500 truncate font-medium">Free delivery thresholds & rates</p>
                  </div>
                  <ChevronLeft size={14} className="text-gray-400 rotate-180 group-hover:text-pink-600 transition-colors" />
                </button>
                ...
                <button
                  onClick={() => {
                    setViewMode("chat");
                    handleSendMessage("refund policy");
                  }}
                  className="w-full flex items-center gap-2.5 p-2.5 rounded-2xl bg-slate-50 hover:bg-pink-50/70 border border-slate-200/70 hover:border-pink-200 transition-all text-left group cursor-pointer"
                >
                  <div className="w-8 h-8 rounded-xl bg-emerald-100/60 text-emerald-600 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                    <RefreshCcw size={16} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-xs text-gray-900 group-hover:text-pink-600 transition-colors">🔄 Return & Refund</h4>
                    <p className="text-[9px] text-gray-500 truncate font-medium">7-day easy returns policy</p>
                  </div>
                  <ChevronLeft size={14} className="text-gray-400 rotate-180 group-hover:text-pink-600 transition-colors" />
                </button>
                ...
                <button
                  onClick={() => {
                    setViewMode("chat");
                    handleSendMessage("contact care");
                  }}
                  className="w-full flex items-center gap-2.5 p-2.5 rounded-2xl bg-slate-50 hover:bg-pink-50/70 border border-slate-200/70 hover:border-pink-200 transition-all text-left group cursor-pointer"
                >
                  <div className="w-8 h-8 rounded-xl bg-purple-100/60 text-purple-600 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                    <Headset size={16} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-xs text-gray-900 group-hover:text-pink-600 transition-colors">💬 Support Care</h4>
                    <p className="text-[9px] text-gray-500 truncate font-medium">24/7 customer assistance & tickets</p>
                  </div>
                  <ChevronLeft size={14} className="text-gray-400 rotate-180 group-hover:text-pink-600 transition-colors" />
                </button>

                {/* 6. Direct Chat with Aarohi */}
                <button
                  onClick={() => setViewMode("chat")}
                  className="w-full flex items-center gap-2.5 p-2.5 rounded-2xl bg-gradient-to-r from-pink-500 to-rose-500 text-white shadow-md shadow-pink-500/20 hover:opacity-95 transition-all text-left group cursor-pointer mt-1"
                >
                  <div className="w-8 h-8 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
                    <MessageSquare size={16} className="text-white" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-extrabold text-xs text-white">🤖 Chat with Aarohi</h4>
                    <p className="text-[9px] text-pink-100 truncate font-medium">Ask any custom questions directly</p>
                  </div>
            ...
            /* VIEW 2: Interactive Chatbot View */
            <>
              {/* Chat Messages Log */}
              <div 
                id="mascot-chat-log"
